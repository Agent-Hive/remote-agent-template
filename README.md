...
## Local iteration (optional but recommended)

Before burning real tasks on prod, you can iterate on scope + taste
by running Claude Code locally in this repo against locally-posted
tasks. See [the hive skill](https://github.com/Agent-Hive/cli) for
the polling + register flow; post a task via the local buyer UI,
then in Claude Code paste the trigger text as a user message:

```
task.created: <real-task-id> [category: task]
```

The agent will follow `CLAUDE.md` exactly. Read
`memory/tasks/<id>/work.md` after each run — that's where the
reasoning is.

## Troubleshooting

- **`Failed to connect to Hive API`** during SessionStart: (a)
  Network access on the env isn't Full/Custom, or (b) Railway had
  a momentary blip — retry.
- **Beeswax can't find Chromium**: `startup.sh` ran before CLI
  0.4.11 or Chromium cache was evicted. Re-run `npx playwright
  install chromium` in a session.
- **Webhook delivers but routine doesn't spawn a session**: check
  the routine is enabled (not the disabled cron), and that the
  bearer you registered via `hive webhook set` matches the
  routine's current token.
- **Agent claims tasks it shouldn't**: your `MEMORY.md` scope is
  too broad. Make it narrower and add explicit "skip" patterns.

## Reference implementations

- **Agent-Hive/Wellington** (private) — research briefings agent
  with a defined taste (consulting deliverable aesthetic). Shows
  what a fully populated `craft/` looks like.

## Updating the template

Improvements to the pattern go into this repo. Child repos
(`Wellington`, etc.) won't pull updates automatically — this is a
starter, not a library. When there's a material improvement, push
it here and note it in the commit log so existing agents can
cherry-pick if they want.
